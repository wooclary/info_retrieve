...
def build(stats_tree_root):
    ...
    :return: 摘要树根结点, 索引
    """
    # 接收 (node, parent_tag, level) 三元组
    q = Queue([(stats_tree_root, '', 0)])
    # 摘要树的层次索引 [{tag: synopsis_node}]
    level_index = []

    # 广度优先遍历
    while q:
        # 出队
        node, parent_tag, level = q.pop()
        ...
        if len(level_index) <= level:
            # 创建新的摘要结点，并更新index
            synopsis_node = SynopsisNode(tag)
            level_index.append({tag: synopsis_node})
            ...
            if tag in index:
                # 直接将结点添加在摘要结点下
                index[tag].add_corresponding_ele(node)
            else:
                # 否则创建新的摘要结点
                synopsis_node = SynopsisNode(tag)
                index[tag] = synopsis_node
                synopsis_node.add_corresponding_ele(node)
        # 若在非0的层创建了新的摘要结点, 则更新其parent的children列表
        if level and synopsis_node:
            parent_synopsis_node = level_index[level-1][parent_tag]
            parent_synopsis_node.add_child(synopsis_node)

        # 入队
        # 为所有子结点构造 (node, parent_tag, level) 三元组
        child_items = [(child, tag, level+1) for child in node.children]
        q.add(*child_items)
...
def calculate(level_index):
    ...
    for level in level_index:
        for tag, synopsis in level.items():
