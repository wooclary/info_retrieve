...
Attachment.prototype.isImg = function(){
	...
	for ( var i = 0; i < imgType.length; i++) {
		if(filename.endsWith("." + imgType[i])){
			return true;
		}
	}
	return false;
...
Attachment.prototype.toString = function(isShowLink, isShowDelete, onlineEdit, width){
    var contextPath = v3x.baseURL;
    var str = "";
    str += "<div id='attachmentDiv_" + this.fileUrl + "' class='attachment_block' style='font-size:12px; float: left;height: " + attachmentConstants.height + "px; line-height: 14px;' noWrap>"
    if(this.type != 1){
...
function preViewDialog(url){
    ...
    if(getA8Top().isCtpTop || getA8Top()._ctxPath){
        getA8Top().addattachDialog = getA8Top().$.dialog({
        title: v3x.getMessage("V3XLang.OfficeTrans_view"),
        transParams:{'parentWin':window},
        url     : url,
        width   : 1280,
        height  : 800,
      ...
      on : {
          instanceReady : function( ev ) { 
              ...
              function resizeEditor(){
                  ...
                  if (document.getElementById("editerDiv")) {
                	  var _height = document.getElementById("editerDiv").style.height;
                	  _height = _height.replace("px","");
                	  if(_height==""){
                		  _height = height;
                	  }
                	  CKEDITOR.instances['content'].ui.space( 'contents' ).setStyle( 'height', _height +'px' );
                  } else {
...
function pigeonhole(appName, ids, atts, validAcl,type,collBackFuns){
    ...
    if (typeof(collBackFuns) != 'undefined') {
    	getA8Top().pigeonholeWin = getA8Top().$.dialog({
            title:v3x.getMessage("V3XLang.doc_v3x_pigeonhole_title"),
            transParams:{'parentWin':window},
...
//  } 
        var member_view_win = new MxtWindow({
            ...
            isDrag:true,
            buttons: [{
            text: v3x.getMessage("V3XLang.close"),
            handler: function () {
...
MxtWindow.prototype.getPanel = function(){
    ...
    if (this.buttons != null && this.buttons.length > 0) {
        ...
        for (var i = 0; i < this.buttons.length; i++) {
            ...
            botton.className="button-default-2 margin_r_5";
            
            if (jsonTemp.emphasize) {
                botton.className="button-default_emphasize margin_r_5";
            }
            if (jsonTemp.text) {
...
function showCtpLocation(code,options){
  ...
  if(options && options.html){
    if(top.showLocation){
        var icon = top.currentSpaceType || "personal";
        var skinPathKey = top.skinPathKey || "defaultV51";
        var html = '<span class="nowLocation_ico"><img src="'+v3x.baseURL+'/main/skin/frame/'+skinPathKey+'/menuIcon/'+icon+'.png"></span>';
  ...
  if(top.$){
    ...
    var menus = findMenu(code);
      if(menus.length>0){
          ...
          for (var i = 0; i < menus.length; i++) {
            if(menus[i].url){
              items.push('<a href="javascript:void(0)" class="hand" onclick="showMenu(\'' +v3x.baseURL+ menus[i].url + '\')">' + menus[i].name + '</a>');
            }else{
