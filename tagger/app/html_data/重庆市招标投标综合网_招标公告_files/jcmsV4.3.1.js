...
var site = new Object();
site.Name = '重庆市招标投标综合网';
site.Url = 'http://www.cqzb.gov.cn';
site.Dir = '/';
site.IsAnnounce = '0';
site.CookieDomain = '';
site.CookiePrev = 'jcms431';
...
var ___JSON_Modules = {recordcount: 8, table: [{no: 0, id: 1, title: '文章', type: 'article', fieldvalues: 'title,summary', fieldtexts: '标题,简介'},{no: 1, id: 2, title: '图片', type: 'photo', fieldvalues: 'title,summary', fieldtexts: '标题,简介'},{no: 2, id: 3, title: '下载', type: 'soft', fieldvalues: 'title,summary', fieldtexts: '标题,简介'},{no: 3, id: 4, title: '视频', type: 'video', fieldvalues: 'title,summary', fieldtexts: '标题,简介'},{no: 4, id: 7, title: '招标公告', type: 'zbgg', fieldvalues: 'title,xmbh,ssqx', fieldtexts: '标题,项目,所属区县'},{no: 5, id: 8, title: '比选公告', type: 'bxgg', fieldvalues: 'title,xmbh,ssqx', fieldtexts: '标题,简介'},{no: 6, id: 9, title: '代理机构', type: 'dljg', fieldvalues: 'title,address', fieldtexts: '代理机构名称,地址'},{no: 7, id: 10, title: '违法行为公告', type: 'wfgg', fieldvalues: 'title,address', fieldtexts: '当事人的名称,项目所在地'}]}
...
function _jcms_Dialog(styletype, title, width, height, iswhat, showCloseBox){
	...
	this.init = function(_showTitleBar){
		...
		__('dialogFrame') ? __('dialogFrame').src='' : function(){};
		...
		_jcms_WindowMask = __("windowMask");
	...
	//this.show = function(){__('dialogBox') ? function(){} : this.init();_jcms_DialogIsShown=true;this.middle('dialogBox');}
	this.show = function(){__('dialogBox') ? function(){} : this.init();_jcms_DialogIsShown=true;this.middle('dialogBox');this.shadow();this.middle('dialogBoxShadow');_jcms_OperatorPlus();}
	...
	this.open = function(_sUrl, _returnFunc, _sMode){
		...
		//if(!_sMode || _sMode == "no" || _sMode == "yes"){
			__("dialogBody").innerHTML = "<iframe id='dialogFrame' name='dialogFrame' src='"+_sUrl+"' width='" + oWidth + "' height='" + oHeight + "' frameborder='no' border='0' marginwidth='0' marginheight='0' scrolling='" + _sMode + "'></iframe>";
...
function ExecuteJsonData(oJsonUrl, oTemplateUrl, oDiv, oFileName, oPluginName, oFileType)
{
	$.ajax({
		...
		url:            site.Dir+"ajax/catchjson.aspx?file="+oJsonUrl,
...
function ExecuteTemplate(oJsonData, oTemplateUrl, oDiv, oFileName, oPluginName, oFileType)
{
	$("#"+oDiv).setTemplateURL(oTemplateUrl+"?time="+(new Date().getTime()), null, {filter_data: true});
	$("#"+oDiv).processTemplate(oJsonData);
	if(oFileName!=null)
	{
		SaveHtml2File($("#"+oDiv).html(), oFileName, oPluginName, oFileType);
	}
...
function SaveHtml2File(oHtml, oFileName, oPluginName, oFileType)
{
	...
	$.ajax({
		type:		"post",
		dataType:	"json",
		data:		"html="+encodeURIComponent(HTML2LowerCase(oHtml))+"&filename="+oFileName+"&filetype="+oFileType+"&pluginname="+oPluginName+"&time="+(new Date().getTime()),
		url:		site.Dir + "ajax/html2file.aspx",
		error:		function(XmlHttpRequest,textStatus, errorThrown){if(XmlHttpRequest.responseText!=""){alert(XmlHttpRequest.responseText);}},
		success: function(){}
	});
}
	    ...
	    success: function (d) {
	        if (d.result != "1" && w == 1) {
	            ...
	            if (d.result == "1") {//成功登录
	                ...
	                var _ajaxLoginbar = '\
						<ul>\
							<li class="user"><a href="' + site.Dir + 'user/index_default.aspx" title="进入个人中心" target="_blank">' + d.workunit + '</a></li>\
							<li><a href="' + site.Dir + 'passport/logout.aspx?userkey=' + d.userkey + '">[退出]</a><br /></li>\
						<li class="hr">|</li><li><a href="' + site.Dir + 'user/index_default.aspx" target="_blank">进入我的中心</a></li>' + _adminBar + '\
						</ul>\
					';
	                $("#user_status2").html(_ajaxLoginbar);
	                $("#user_status1").hide();
	                $("#xxxg").show();
	            }
	        }
